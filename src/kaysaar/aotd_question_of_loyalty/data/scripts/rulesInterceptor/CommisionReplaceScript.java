        ...
        if (Global.getSector().getCampaignUI().isShowingDialog()) {
            ...
            if (dialog != null) {
                if (dialog.getOptionPanel().hasOption("cmsn_askCommission")&&!isInInteraction) {
                    ...
                    if(FactionHatredManager.get().doesFactionHatePlayer(dialog.getInteractionTarget().getFaction().getId())){
                        dialog.getOptionPanel().setEnabled("aotd_cmsn_askCommission", false);
                        if(FactionHatredManager.get().getRebellions()>=FactionHatredManager.maxRebellions){
                            dialog.getOptionPanel().setTooltip("aotd_cmsn_askCommission","Due to your backstabbing previous commissioners none of factions trust you anymore!");

                        }
                        else{
                            dialog.getOptionPanel().setTooltip("aotd_cmsn_askCommission","Due to our actions this faction won't ever commissions us again!");
